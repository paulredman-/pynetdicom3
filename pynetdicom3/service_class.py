...
class ModalityPerformedProcedureStepServiceClass(ServiceClass):
    ...
    def SCP(self, req, context, info):
        ...
        if isinstance(req, N_CREATE):
            return self._create(req, context, info)
        elif isinstance(req, N_SET):
            return self._set(req, context, info)
        else:
            raise NotImplementedError(
                "The service class processing for this message {} has not been implemented yet".format(type(req))
            )

    def _create(self, req, context, info):
        """
        Parameters
        ----------
    ...
    def _set(self, req, context, info):
        """
        Parameters
        ----------
        req : pynetdicom3.dimse_primitives.N_SET
        ...
        *Status*
        ...
        TODO
        """
        # Build N-SET response primitive
        rsp = N_SET()
        rsp.MessageID = req.MessageID
        rsp.MessageIDBeingRespondedTo = req.MessageID
        rsp.RequestedSOPClassUID = req.RequestedSOPClassUID
        rsp.RequestedSOPInstanceUID = req.RequestedSOPInstanceUID
        ...
        transfer_syntax = context.transfer_syntax[0]
        try:
            modification_list = decode(req.ModificationList,
                                transfer_syntax.is_implicit_VR,
                                transfer_syntax.is_little_endian)
            LOGGER.info('MPPS SCP Request Modification List:')
            LOGGER.info('')
            LOGGER.debug('# DICOM Dataset')
            for elem in modification_list.iterall():
                LOGGER.info(elem)
            LOGGER.info('')
        except Exception as ex:
            LOGGER.error("Failed to decode the request's Modification List dataset.")
            LOGGER.exception(ex)
            # Failure - Unable to Process - Failed to decode Identifier
            rsp.Status = 0xC310
            rsp.ErrorComment = 'Unable to decode the dataset'
        ...
        # Try and run the user's on_n_set callback. The callback should return
        #   the Status as either an int or Dataset, and any failures in the
        #   callback results in 0x0000 'Success'
        try:
            status = self.AE.on_n_set(modification_list, context.as_tuple, info)
            LOGGER.info("Returned: status=", status)
            if isinstance(status, Dataset):
                if 'Status' not in status:
                    raise AttributeError("The 'status' dataset returned by "
                ...
                raise TypeError("Invalid 'status' returned by 'on_n_set'")

            # Check Status validity
            if not self.is_valid_status(rsp.Status):
                LOGGER.warning("Unknown 'status' value returned by 'on_n_set' "
                               "callback - 0x{0:04x}".format(rsp.Status))
        except Exception as ex:
            LOGGER.exception(ex)
            LOGGER.error("Exception in the 'on_n_set' callback, responding "
                         "with default 'status' value of 0x0000 (Success).")
            rsp.Status = 0x0000
